...
{
    public class SemanticKernelOrchestrator : IAIBrainOrchestrator
    {
        ...
        public async Task<AIBrainChatResponse> ChatAsync(AIBrainChatRequest request, CancellationToken cancellationToken = default)
        {
            ...
            ConductorResult? conductorDecision = null;
            if (request.Mode == ChatMode.PublicBooking)
            {
                conductorDecision = await _bookingConductor.DecideAsync(sessionId, request.Message, request, cancellationToken);
                ApplyConductorStateToSession(sessionState, conductorDecision.State);
            }
            ...
            var knowledgePlugin = new WebHomestay.Services.AI.Plugins.KnowledgeGraphPlugin(_context, embeddingService, sessionState, request);
            var semanticSearchPlugin = new WebHomestay.Services.AI.Plugins.SemanticSearchPlugin(_context, embeddingService, sessionState);
            ...
            {
                var studioTrace = await SaveTraceAsync(
                    sessionId,
                    request.Message,
            ...
            if (conductorDecision != null)
            {
                ...
                if (shouldHandleDeterministically)
                {
                    ...
                    var conductorTrace = await SaveTraceAsync(
                        sessionId,
                        request.Message,
            ...
            string provider = string.IsNullOrWhiteSpace(_aiOptions.Provider) ? "groq" : _aiOptions.Provider.ToLower();
            try
            {
                ...
                answerContent = result.Content ?? string.Empty;
            }
            catch (Exception ex)
            {
                _logger.LogError("Semantic Kernel execution failed: {ErrorType} - {ErrorMessage}", ex.GetType().Name, ex.Message);
                answerContent = $"[DEBUG] Lỗi AI: {ex.Message} (Type: {ex.GetType().Name})\n\nHiện tại hệ thống AI đang có chút sự cố kết nối, nhưng tôi vẫn có thể hỗ trợ bạn đặt phòng. Vui lòng cho biết chi nhánh, ngày và số khách để tôi kiểm tra nhé.";
                provider = "fallback-rule";
            }

            var trace = await SaveTraceAsync(
                sessionId,
                request.Message,
                "Semantic Kernel Managed",
                bookingPlugin.GetLogs(),
                knowledgePlugin.GetLogs() + "\n" + semanticSearchPlugin.GetLogs(),
                answerContent,
                provider,
                cancellationToken);
        ...
        private Kernel BuildKernel()
        {
            ...
            var provider = string.IsNullOrWhiteSpace(_aiOptions.Provider) ? "groq" : _aiOptions.Provider.ToLowerInvariant();
            var model = string.IsNullOrWhiteSpace(_aiOptions.Model) ? "llama-3.3-70b-versatile" : _aiOptions.Model;

            var builder = Kernel.CreateBuilder();

            if (provider == "groq" || provider == "openrouter" || provider == "9router" || provider == "gemini")
            {
                ...
                else if (!string.IsNullOrWhiteSpace(_aiOptions.Endpoint))
                {
            ...
            {
                // Default fallback to OpenAI standard
        ...
        private static bool IsBookingStartIntent(string message)
        {
            ...
            var bookingWords = new[] { "đặt phòng", "dat phong", "book phòng", "book phong", "muốn đặt", "muon dat", "tìm phòng", "tim phong", "lấy phòng", "lay phong" };
            return bookingWords.Any(word => text.Contains(word))
                && !text.Contains("hủy")
                && !text.Contains("huy");
        }

        private async Task<AIConversationTrace> SaveTraceAsync(
            string sessionId,
            ...
            CancellationToken cancellationToken)
        {
            var trace = new AIConversationTrace
            {
                ...
                GraphReasoningJson = personaSummary,
